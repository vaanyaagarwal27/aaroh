    ...
    <>
      ...
      <div className="verify-page">
        ...
        <div className="verify-content">
          ...
          {/* Edit History */}
          {edits.length > 0 && (
            <section className="vedit-history-section">
              <div className="vsection-header">
                <h2 className="vsection-title">Edit History ({edits.length} change{edits.length !== 1 ? 's' : ''})</h2>
              </div>
              <div className="vedit-history-list">
                {edits.map((e, i) => (
                  <div key={i} className="vedit-card">
                    {e.changes?.map((ch, j) => (
                      <div key={j} className="vedit-change">
                        {ch.field && <p className="vedit-change-field">{ch.field}</p>}
                        <div className="vedit-block vedit-block--old">
                          <span className="vedit-block-label">OLD</span>
                          <p className="vedit-block-text">{ch.original || '—'}</p>
                        </div>
                        <div className="vedit-block vedit-block--new">
                          <span className="vedit-block-label">NEW</span>
                          <p className="vedit-block-text">{ch.updated || '—'}</p>
                        </div>
                      </div>
                    ))}
                    <div className="vedit-footer">
                      <span><strong>Changed by:</strong> {e.edited_by}</span>
                      <span className="vedit-sep">|</span>
                      <span><strong>Reason:</strong> {e.reason}</span>
                      <span className="vedit-sep">|</span>
                      <span className="vedit-time">{e.edited_at}</span>
                    </div>
                  </div>
                ))}
              </div>
